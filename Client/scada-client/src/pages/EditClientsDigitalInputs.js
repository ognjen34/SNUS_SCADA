...
import { GetAllDigital } from '../services/tagService';
import { UpdateUser } from '../services/userService';

function EditClientsDigitalInputs({ onCloseDialog, selectedclient }) {
    ...
    const handleSaveButton = async (e) => {
        ...
        selectedclient['digitalInputs'] = usersAnalogInput
        console.log("Novi brat:")

        console.log(selectedclient)

        try{
            const updatedUser = await UpdateUser(selectedclient['name'], selectedclient['surname'],selectedclient['email'],  selectedclient['password'], selectedclient['analogInputs'].map(obj => obj.id), selectedclient['digitalInputs'])
    ...
    useEffect(() => {
        async function getAnalogInputs() {
            try {
                const response = await GetAllDigital();
        ...
        if (selectedclient) {
            ...
            setUsersAnalogInput(selectedclient['digitalInputs'].map(obj => obj.id));
            console.log("mapapappapap::::")
            console.log(selectedclient['digitalInputs'].map(obj => obj.id))
...
export default EditClientsDigitalInputs;
